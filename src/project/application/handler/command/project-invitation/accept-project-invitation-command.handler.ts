...
import { Cache, CacheEvict, Lock } from '@libs/decorators';
  ...
  implements ICommandHandler<AcceptProjectInvitationCommand>
{
  ...
  @CacheEvict({
    keys: args => [`entity:project-invitation:${args[0].invitationId}`],
    timing: 'after',
  })
  @Lock({
    key: args => `accept-project-project-invitation:${args[0].invitationId}`,
    ttl: 5000,
  })
  @Transactional()
  async execute(
